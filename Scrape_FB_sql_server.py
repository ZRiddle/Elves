...
cursor.executemany("INSERT INTO FB_Post_Dim values (?, ?, ?,?,?, ?, ?, ?, ?, ?,?);",\
                     [tuple(x) for x in posts_df.values])
cnxn.commit()
print 'New post data added to database'
cursor.executemany("INSERT INTO FB_Comment_Dim values (?, ?, ?, ?, ?,?, ?, ?, ?);",\
                 [tuple(x) for x in comments_df.values] )
cnxn.commit()
print 'New comment data added to database'
cursor.executemany("INSERT INTO FB_Like_Dim values (?, ?, ?, ?, ?, ?);",\
...
#            "--vanilla C:/Users/zriddle/Documents/Projects/Projects/~~Old Projects/Sentiment_Analysis/Sentiment_Analysis_Oracle_Update.R")
