...
smallest_ramp = 560         #smallest amount of time allowed to work on ramp
biggest_ramp = 1.01          #multipled by time left in day, allowed to go some % over to ramp
start_ramping_again = 3.1  #threshold to start ramping after doing midrange/overnight/big present
helper_elves = 15            #percent of elves to help with midrange - idea is to stagger start times to ramp differently
                                #1 = 0%, 2 = 50%, 3 = 67%, 4 = 75%
opt_update = 20000          #How often to update the optimization
...
ref_time = datetime.datetime(2014, 12, 11, 0, 0) #Last Present arrives 2014-12-10 23:59:00.000
...
if __name__ == '__main__':
    ...
    #Get Small Toy List
    sql2 = ("""SELECT ToyID, cast(Duration as int) Duration
                FROM [RM_Sandbox].[dbo].[toys_elves]
                WHERE Duration<2844
                ORDER BY duration, Arrival_time """)
    cursor.execute(sql2)
    ...
    while len([k for k in small_toy_1.keys() if k<180])>0:
        ...
        if rating>=2.1:
            ...
            T0 = B0/rating + B1/r0 + 4*Bm + rest_of_day                                   #Doing smallest then biggest
            ...
            T00= B0/rating + Bm/(r0) + B1/(overnight*r0*overnight_delta) + rest_of_day    #Doing smallest then overnight then biggest
        ...
        #Ramp if below threshold for current bad presents
            ...
            if flag and min_day/(abs(math.ceil(pres_row/rating)-min_day)+1)<good_vs_bad_ratio and min_day<599:
                ...
                #Work Present
                current_present = small_toy_1[pres_row].pop()
        ...
        #if we can keep ramping, then ramp! Only ramp if present exists to work on for at least 500 minutes
        ...
        #Have elf 1 doing those midrange presents
            ...
            if flag and min_day/(abs(math.ceil(pres_row/rating)-min_day)+1)<break_even_cutoff:
                ...
                #Work Present
                current_present = midrange_toy[pres_row].pop()
        ...
        elif current_elf.id<=midrange_elfs and max(small_toy_1.keys())>2000:
        ...
        elif rating>=2.1 and (Tm<T1 or T00<T1 or T0<T1):
            if Tm<T00 and Tm<T0 and Bm>0:
                if (Bm/rating + min_day)>(60*34) and min_day<599:
                    ...
                    current_present = overnight_toy[Bm].pop()
        ...
        #Else, do big present, kill elf, start over
            ...
            if rating<ramp_cutoff and len([k for k in small_toy_1.keys() if (k<=605*rating and k>=565*rating)])>0\
            and bad_toys[0][1]/rating>(bad_toys[0][1]/(rating*1.2)+min_day) and min_day<599:
                #skip elf and push him to the next day
                current_elf.next_available_time = current_elf.next_available_time+min_day+840
                #push Elf to heapq
                heapq.heappush(myelves, (current_elf.next_available_time, current_elf))
                ...
                if len(bad_toys)%opt_update==0 and bad_toys[0][1]>9000:
                    ...
                    for k in range(len(bad_toys)):
                        if bad_toys[k][1]>lower_bound and bad_toys[k][1]<20000:
                            Sum1 += math.sqrt(1.0*bad_toys[k][1]/T01)
                            n += 1

                    #Next bad_present should get this many hours
                    h = (cost + n/.022 - Sum1/.022)/Sum1
                    #Next target effencSiency is .25*1.02^h
                    rating_threshold = .25*(1+.022*h)
                    #update lower bound
                    lower_bound = T01/math.pow((1+.022*h),2)
    ...
    #Write to CSV File
    soln_file = os.path.join(os.getcwd(), 'output18.csv')
