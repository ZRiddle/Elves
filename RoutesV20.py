...
smallest_ramp = 550         #smallest amount of time allowed to work on ramp
biggest_ramp = 1.04          #multipled by time left in day, allowed to go some % over to ramp
...
if __name__ == '__main__':
    ...
    while len([k for k in small_toy_1.keys() if k<180])>0:
        ...
        elif first_year_bool and rating>=4.0 and max(small_toy_1.keys())>=2390 and min_day==600:
        ...
        elif current_elf.id<=midrange_elfs and max(small_toy_1.keys())>1500:
    ...
    #Write to CSV File
    soln_file = os.path.join(os.getcwd(), 'output24.csv')
